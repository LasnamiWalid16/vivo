...
def test_get_bigquery_job_query_as_json(mock_credentials, mock_bigquery_client):
    ...
    mock_row_1 = {"name": "Alice", "age": 30}
    mock_row_2 = {"name": "Bob", "age": 25}
    ...
    result = get_bigquery_job_query_as_json(
        "SELECT name, age FROM table", "fake_path.json"
    )

    assert result == mock_result
    mock_bigquery_client.assert_called_once()
    mock_client_instance.query.assert_called_once_with("SELECT name, age FROM table")
