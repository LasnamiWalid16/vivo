...
from config import SERVICE_ACCOUNT_FILE, PROJECT_ID
...
def get_drug_journals(drug: str) -> list[str]:
    ...
    full_table_id = f"{PROJECT_ID}.{dataset_id}.{table_id}"
    query = f"SELECT * FROM `{full_table_id}`"
    data = load_bigquery_query_as_json(query, SERVICE_ACCOUNT_FILE)
    ...
    # remove drugs mentionned in cliniv=cal trials
    result = list(set(list(reference_journal_drugs_pubmed)) - set(list(reference_journal_drugs_clinical)))
    if drug in result:
        result.remove(drug)
